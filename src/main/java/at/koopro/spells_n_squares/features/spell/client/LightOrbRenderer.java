...
public class LightOrbRenderer extends EntityRenderer<LightOrbEntity, EntityRenderState> {
    private static final Identifier GLOW_TEXTURE = Identifier.fromNamespaceAndPath(SpellsNSquares.MODID, "textures/misc/lumos_white.png");
    ...
    public void submit(EntityRenderState renderState, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        collector.submitCustomGeometry(
            ...
            (pose, buffer) -> {
                ...
                ps.last().set(pose);
                ps.pushPose();
                ps.translate(0.0f, 0.1f, 0.0f);
                ps.mulPose(Axis.YP.rotation(time * 0.15f));
                ps.mulPose(Axis.XP.rotation(Mth.sin(time * 0.08f) * 0.07f));

                renderCube(buffer, ps, 0.06f, 0xFFFFFFFF);
                renderCube(buffer, ps, 0.10f, 0x60FFE080);
