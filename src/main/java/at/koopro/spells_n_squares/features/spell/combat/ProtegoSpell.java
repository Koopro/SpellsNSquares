...
public class ProtegoSpell implements Spell {
    ...
    public boolean cast(Player player, Level level) {
        ...
        ShieldOrbEntity shield = new ShieldOrbEntity(
            at.koopro.spells_n_squares.features.spell.manager.SpellEntityRegistry.SHIELD_ORB.get(),
            level
        );
        Vec3 pos = player.position().add(0, player.getBbHeight() * 0.5, 0);
        shield.setPos(pos.x, pos.y, pos.z);
        shield.setOwner(player); // Set the owner so shield follows and protects the player
