...
public class LocationCompassItem extends Item {
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (waypoints.isEmpty()) {
            serverPlayer.sendSystemMessage(Component.literal("No waypoints set"));
        } else {
            serverPlayer.sendSystemMessage(Component.literal("Waypoints:"));
            for (WaypointSystem.Waypoint waypoint : waypoints) {
                serverPlayer.sendSystemMessage(Component.literal("  - " + waypoint.name() + 
                    " at " + waypoint.position().getX() + ", " + waypoint.position().getY() + ", " + waypoint.position().getZ()));
            }
        }
