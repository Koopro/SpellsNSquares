...
import at.koopro.spells_n_squares.features.creatures.neutral.StreelerEntity;
...
/**
 * Renderer for Streeler entity - giant color-changing snail.
 */
public class StreelerRenderer extends EntityRenderer<StreelerEntity, EntityRenderState> {
    ...
    @Override
    public void extractRenderState(StreelerEntity entity, EntityRenderState state, float partialTick) {
    ...
    public void submit(EntityRenderState renderState, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        collector.submitCustomGeometry(
            ...
            (pose, buffer) -> {
                ...
                // Giant snail body (color changes - using purple as placeholder)
                int color = 0xFF9932CC; // Purple (changes hourly)
                RendererUtils.renderCube(buffer, ps, half * 0.7f, color);
