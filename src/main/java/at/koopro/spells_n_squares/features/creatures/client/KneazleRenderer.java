...
import at.koopro.spells_n_squares.features.creatures.companion.KneazleEntity;
...
/**
 * Renderer for Kneazle entity - an intelligent cat-like creature.
 */
public class KneazleRenderer extends EntityRenderer<KneazleEntity, EntityRenderState> {
    
    public KneazleRenderer(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    public void extractRenderState(KneazleEntity entity, EntityRenderState state, float partialTick) {
    ...
    public void submit(EntityRenderState renderState, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        collector.submitCustomGeometry(
            ...
            (pose, buffer) -> {
                ...
                // Cat-like body (spotted/patterned)
                RendererUtils.renderCube(buffer, ps, half * 0.8f, 0xFFD4A574);
                
                // Ears
                ps.pushPose();
                ps.translate(half * 0.3f, half * 0.4f, 0);
                RendererUtils.renderCube(buffer, ps, half * 0.2f, 0xFFB8945A);
                ps.popPose();
