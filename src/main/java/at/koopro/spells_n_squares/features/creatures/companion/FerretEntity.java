...
/**
 * Ferret - a familiar companion creature, playful and quick.
 */
public class FerretEntity extends BaseTamableCreatureEntity {
    
    public FerretEntity(EntityType<? extends FerretEntity> type, Level level) {
        super(type, level);
    }
        ...
        if (!hasOwner() && !stack.isEmpty()) {
            if (stack.is(net.minecraft.world.item.Items.BEEF) || 
                ...
                if (!this.level().isClientSide() && player instanceof net.minecraft.server.level.ServerPlayer serverPlayer) {
                    ...
                    this.level().playSound(null, this.blockPosition(), SoundEvents.VILLAGER_YES, SoundSource.NEUTRAL, 0.8f, 1.4f);
                    serverPlayer.sendSystemMessage(Component.translatable("message.spells_n_squares.ferret.tamed"));
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob partner) {
        return null; // Ferrets don't breed
    }
}
