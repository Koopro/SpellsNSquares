...
import at.koopro.spells_n_squares.features.creatures.companion.SnakeEntity;
...
/**
 * Renderer for Snake familiar entity - a snake companion.
 */
public class SnakeFamiliarRenderer extends EntityRenderer<SnakeEntity, EntityRenderState> {
    
    public SnakeFamiliarRenderer(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    public void extractRenderState(SnakeEntity entity, EntityRenderState state, float partialTick) {
    ...
    public void submit(EntityRenderState renderState, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        collector.submitCustomGeometry(
            ...
            (pose, buffer) -> {
                ...
                // Long body (green/brown colored)
                RendererUtils.renderCube(buffer, ps, half * 0.9f, 0xFF4A6B2A);
                
                // Head
                ps.pushPose();
                ps.translate(0, 0, -half * 0.5f);
                RendererUtils.renderCube(buffer, ps, half * 0.4f, 0xFF3A5B1A);
                ps.popPose();
