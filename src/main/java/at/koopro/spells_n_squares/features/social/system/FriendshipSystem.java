package at.koopro.spells_n_squares.features.social.system;

import at.koopro.spells_n_squares.features.social.data.SocialData;
 ...
 * 
 * <p><b>Thread Safety:</b> All static collections ({@code friendshipPoints}, {@code activeQuests}) 
 * are accessed only from the main server thread during game ticks. These collections are not thread-safe, 
 * but thread safety is not required as Minecraft's game logic runs on a single thread. If these collections 
 * are ever accessed from multiple threads in the future, they should be converted to thread-safe collections 
 * (e.g., {@code ConcurrentHashMap}).
 */
public final class FriendshipSystem {
    ...
    public static Set<String> getFriendshipBenefits(FriendshipLevel level) {
        ...
        switch (level) {
            case ACQUAINTANCE:
                // No benefits at acquaintance level
                break;
            case FRIEND:
     ...
     * @param playerUUID The UUID of the player disconnecting
     */
    public static void clearPlayerData(UUID playerUUID) {
        ...
        try {
            // Clear friendship points (remove entries where this player is player1)
            friendshipPoints.remove(playerUUID);
            
            // Also remove entries where this player is player2
            friendshipPoints.values().removeIf(map -> map != null && map.containsKey(playerUUID));
            
            // Clear active quests
            activeQuests.remove(playerUUID);
        } catch (Exception e) {
            com.mojang.logging.LogUtils.getLogger().error("Error cleaning up friendship data for player {}: {}", 
