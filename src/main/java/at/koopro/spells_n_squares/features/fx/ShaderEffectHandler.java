...
public class ShaderEffectHandler {
    ...
    public static boolean isShaderLoaded(Identifier shaderId) {
        ...
        if (LUMOS_ORB_SHADER.equals(shaderId)) {
            ...
            available = pipeline != null && PostProcessingManager.isPipelineValid(pipeline);
        } else if (ENERGY_BALL_SHADER.equals(shaderId)) {
            RenderPipeline pipeline = PostProcessingManager.getEnergyBallRenderPipeline();
            available = pipeline != null && PostProcessingManager.isPipelineValid(pipeline);
        ...
        } else if (POLAROID_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(POLAROID_POST_SHADER);
        } else if (RETRO_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(RETRO_POST_SHADER);
        } else if (BLACK_AND_WHITE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(BLACK_AND_WHITE_POST_SHADER);
        } else if (SATURATED_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(SATURATED_POST_SHADER);
        } else if (GLITCH_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(GLITCH_POST_SHADER);
        } else if (KALEIDOSCOPE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(KALEIDOSCOPE_POST_SHADER);
        } else if (RGB_SHIFT_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(RGB_SHIFT_POST_SHADER);
        } else if (WAVE_DISTORTION_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(WAVE_DISTORTION_POST_SHADER);
        } else if (BLOOM_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(BLOOM_POST_SHADER);
        } else if (EDGE_DETECTION_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(EDGE_DETECTION_POST_SHADER);
        } else if (PIXELATION_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(PIXELATION_POST_SHADER);
        } else if (HEAT_HAZE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(HEAT_HAZE_POST_SHADER);
        } else if (COLOR_CYCLE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(COLOR_CYCLE_POST_SHADER);
        } else if (MIRROR_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(MIRROR_POST_SHADER);
        } else if (NOISE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(NOISE_POST_SHADER);
        } else if (ZOOM_BLUR_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(ZOOM_BLUR_POST_SHADER);
        } else if (UNDERWATER_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(UNDERWATER_POST_SHADER);
        } else if (DRUNK_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(DRUNK_POST_SHADER);
        } else if (MATRIX_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(MATRIX_POST_SHADER);
        } else if (OLD_TV_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(OLD_TV_POST_SHADER);
        } else if (XRAY_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(XRAY_POST_SHADER);
        } else if (THERMAL_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(THERMAL_POST_SHADER);
        } else if (CARTOON_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(CARTOON_POST_SHADER);
        } else if (OIL_PAINTING_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(OIL_PAINTING_POST_SHADER);
        } else if (OLD_FILM_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(OLD_FILM_POST_SHADER);
        } else if (ACID_TRIP_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(ACID_TRIP_POST_SHADER);
        } else if (OUTLINE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(OUTLINE_POST_SHADER);
        } else if (FOG_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(FOG_POST_SHADER);
        } else if (SHARPEN_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(SHARPEN_POST_SHADER);
        } else if (MOTION_BLUR_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(MOTION_BLUR_POST_SHADER);
        } else if (DEPTH_OF_FIELD_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(DEPTH_OF_FIELD_POST_SHADER);
        } else if (LENS_FLARE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(LENS_FLARE_POST_SHADER);
        } else if (VIGNETTE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(VIGNETTE_POST_SHADER);
        } else if (CONTRAST_BOOST_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(CONTRAST_BOOST_POST_SHADER);
        } else if (PULSING_GLOW_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(PULSING_GLOW_POST_SHADER);
        } else if (SCROLLING_STRIPES_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(SCROLLING_STRIPES_POST_SHADER);
        } else if (WARPING_VORTEX_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(WARPING_VORTEX_POST_SHADER);
        } else if (PARTICLE_RAIN_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(PARTICLE_RAIN_POST_SHADER);
        } else if (COLOR_WAVE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(COLOR_WAVE_POST_SHADER);
        } else if (RIPPLE_EFFECT_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(RIPPLE_EFFECT_POST_SHADER);
        } else if (BREATHING_VIGNETTE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(BREATHING_VIGNETTE_POST_SHADER);
        } else if (ANIMATED_NOISE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(ANIMATED_NOISE_POST_SHADER);
        } else if (ROTATING_KALEIDOSCOPE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(ROTATING_KALEIDOSCOPE_POST_SHADER);
        } else if (ENERGY_PULSE_POST_SHADER.equals(shaderId)) {
            available = PostProcessingManager.isPostProcessingShaderAvailable(ENERGY_PULSE_POST_SHADER);
        } else {
    ...
    public static void triggerShaderEffect(String effectType, float intensity) {
        ...
        switch (effectType.toLowerCase()) {
            ...
            case "bw":
                triggerBlackAndWhite(intensity);
                break;
            case "polaroid":
                triggerPolaroid(intensity);
                break;
            case "retro":
            case "vintage":
                triggerRetro(intensity);
                break;
            case "saturated":
            case "saturation":
                triggerSaturated(intensity);
                break;
            case "glitch":
            case "corruption":
            ...
            case "kaleidoscope":
            case "kaleido":
                triggerKaleidoscope(intensity);
                break;
            case "rgb_shift":
            case "rgb":
                triggerRgbShift(intensity);
                break;
            case "wave":
            case "wave_distortion":
            case "ripple":
                triggerWaveDistortion(intensity);
                break;
            case "bloom":
            case "glow":
            ...
            case "edge_detection":
            case "outline":
                triggerEdgeDetection(intensity);
                break;
            case "pixelation":
            case "pixel":
                triggerPixelation(intensity);
                break;
            case "heat_haze":
            case "heat":
            case "haze":
                triggerHeatHaze(intensity);
                break;
            case "color_cycle":
            case "cycle":
            case "rainbow":
                triggerColorCycle(intensity);
                break;
            case "mirror":
            case "flip":
                triggerMirror(intensity);
                break;
            case "noise":
            case "grain":
            case "static":
                triggerNoise(intensity);
                break;
            case "zoom_blur":
            case "zoom":
            case "radial_blur":
                triggerZoomBlur(intensity);
                break;
            case "chromatic":
            ...
            case "pulse_glow":
            case "glow_pulse":
                triggerPulsingGlow(intensity);
                break;
            case "scrolling_stripes":
            case "stripes":
            case "scrolling":
                triggerScrollingStripes(intensity);
                break;
            case "warping_vortex":
            case "vortex":
            case "warp":
                triggerWarpingVortex(intensity);
                break;
            case "particle_rain":
            case "particles":
            case "rain":
                triggerParticleRain(intensity);
                break;
            case "color_wave":
            case "wave_color":
                triggerColorWave(intensity);
                break;
            case "ripple_effect":
            case "ripples":
                triggerRippleEffect(intensity);
                break;
            case "breathing_vignette":
            case "breathing":
                triggerBreathingVignette(intensity);
                break;
            case "animated_noise":
            case "moving_noise":
                triggerAnimatedNoise(intensity);
                break;
            case "rotating_kaleidoscope":
            case "kaleidoscope_rotate":
                triggerRotatingKaleidoscope(intensity);
                break;
            case "energy_pulse":
            case "energy_wave":
                triggerEnergyPulse(intensity);
                break;
            default:
                // Unknown effect type
                break;
