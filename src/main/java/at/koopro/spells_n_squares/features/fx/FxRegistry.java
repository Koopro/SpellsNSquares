...
public class FxRegistry {
    ...
    public static final DeferredHolder<Block, EnergyBallBlock> ENERGY_BALL = BLOCKS.register(
        "energy_ball",
        id -> new EnergyBallBlock(RegistryHelper.createBlockProperties(id).strength(0.5f).noOcclusion())
    );
    
    public static final DeferredItem<BlockItem> ENERGY_BALL_ITEM = ITEMS.register(
        "energy_ball",
        id -> new BlockItem(ENERGY_BALL.value(), RegistryHelper.createItemProperties(id))
    );
