...
public class ShaderEffectIntegrationHandler {
    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        if (mc.level == null || mc.player == null || !Config.areShaderEffectsEnabled()) {
            return;
        }
        ...
        checkDementorProximity(player);
        
        // Check for status effect-based shader effects
        checkStatusEffectShaders(player);
        
        // Check for block-based shader effects
        checkBlockShaderEffects(player);
    }
