package at.koopro.spells_n_squares.features.playerclass.system;
 ...
 * 
 * <p><b>Thread Safety:</b> The static collection {@code playerClasses} is accessed only from the main server thread 
...
public class PlayerClassManager {
    ...
    // Using HashMap with UUID keys to prevent memory leaks (Player objects should not be used as map keys)
    // Thread safety: Accessed only from main server thread
    private static final Map<UUID, Set<PlayerClass>> playerClasses = new HashMap<>();
    ...
    public static Set<PlayerClass> getPlayerClasses(Player player) {
        ...
        Set<PlayerClass> cached = playerClasses.get(player.getUUID());
        if (cached != null) {
    ...
    public static boolean addPlayerClass(Player player, PlayerClass playerClass, String acquiredBy) {
        ...
        playerClasses.put(player.getUUID(), new HashSet<>(classes));
    ...
    public static boolean removePlayerClass(Player player, PlayerClass playerClass) {
        ...
        if (classes.isEmpty()) {
            ...
            playerClasses.put(playerUUID, new HashSet<>(classes));
        }
     ...
     * Primary class is determined by priority: BASE > ROLE > TRANSFORMATION > ALIGNMENT > ORGANIZATION > BLOOD_STATUS
     * 
     * @param classes The set of classes to determine primary from
     * @return The primary class, or NONE if classes is null or empty
     */
    public static PlayerClass getPrimaryClass(Set<PlayerClass> classes) {
    ...
    public static void clearPlayerData(Player player) {
        if (player != null) {
            playerClasses.remove(player.getUUID());
        }
    }
