package at.koopro.spells_n_squares.item.misc;

import at.koopro.spells_n_squares.core.registry.ModSounds;
import at.koopro.spells_n_squares.item.misc.client.RubberDuckItemRenderer;
 ...
 * Implements GeoItem to enable animated 3D model rendering.
 */
public class RubberDuckItem extends Item implements GeoItem {
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        if (!level.isClientSide() && level instanceof ServerLevel serverLevel) {
            // Trigger animation - this syncs to all clients automatically
            triggerAnim(player, GeoItem.getOrAssignId(player.getItemInHand(hand), serverLevel), "Activation", "activate");

            // Play custom rubber duck squeak sound for all nearby players (broadcasts to all clients)
        ...
        // On client, return PASS to allow server to process
