package at.koopro.spells_n_squares.services.spell.internal;

import at.koopro.spells_n_squares.core.util.dev.DevLogger;
import at.koopro.spells_n_squares.features.spell.manager.SpellManager;
import at.koopro.spells_n_squares.services.spell.api.ISpellManager;
...
public class SpellService implements ISpellManager {
    ...
    public boolean castSpellInSlot(Player player, Level level, int slot) {
        DevLogger.logMethodEntry(this, "castSpellInSlot", 
            "player=" + (player != null ? player.getName().getString() : "null") +
            ", slot=" + slot);
        
        if (player == null) {
            LOGGER.warn("Attempted to cast spell from slot {} for null player", slot);
            DevLogger.logWarn(this, "castSpellInSlot", "Player is null");
            DevLogger.logMethodExit(this, "castSpellInSlot", false);
            return false;
        }
        if (level == null) {
            LOGGER.warn("Attempted to cast spell from slot {} for player {} in null level", 
                slot, player.getName().getString());
            DevLogger.logWarn(this, "castSpellInSlot", "Level is null");
            DevLogger.logMethodExit(this, "castSpellInSlot", false);
            return false;
        }
        if (!isValidSlot(slot)) {
            LOGGER.warn("Attempted to cast spell from invalid slot {} for player {}", slot, player.getName().getString());
            DevLogger.logWarn(this, "castSpellInSlot", "Invalid slot: " + slot);
            DevLogger.logMethodExit(this, "castSpellInSlot", false);
            return false;
        }
        try {
            boolean result = SpellManager.castSpellInSlot(player, level, slot);
            DevLogger.logMethodExit(this, "castSpellInSlot", result);
            return result;
        } catch (Exception e) {
            LOGGER.error("Error casting spell from slot {} for player {}: {}", 
                slot, player.getName().getString(), e.getMessage(), e);
            DevLogger.logError(this, "castSpellInSlot", 
                "Error casting spell from slot " + slot, e);
            DevLogger.logMethodExit(this, "castSpellInSlot", false);
            return false;
        }
    }
