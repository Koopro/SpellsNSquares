...
void main() {
    ...
    // Apply outline color
    vec3 outlineColor = vec3(OutlineColorR, OutlineColorG, OutlineColorB);
    vec3 outlined = mix(InTexel.rgb, outlineColor, edge * Intensity);
    
    fragColor = vec4(clamp(outlined, 0.0, 1.0), InTexel.a);
}
