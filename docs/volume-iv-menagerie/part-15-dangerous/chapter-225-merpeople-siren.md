...
title: 'Chapter 225: Merpeople (Siren)'
description: Merpeople (Siren) - Implementation details and reference
tags:
  - iv menagerie
  - 15 dangerous
---
...
The **Merpeople (Siren)** is a XXXX (Dangerous) magical creature that inhabits the wizarding world. Humanoid water creatures This creature plays an important role in the ecosystem and provides various interactions for players, including combat encounters, taming opportunities, and resource gathering.

This chapter documents the complete implementation of the Merpeople (Siren), including AI behaviors, spawning mechanics, combat systems, taming requirements, and all creature-specific properties that make it unique in the mod.

## Implementation Details

### Creature Classification

The Merpeople (Siren) is classified as **XXXX (Dangerous)** according to the Ministry of Magic's classification system. This classification determines how dangerous the creature is and what precautions players should take when encountering it.
...
The Merpeople (Siren) uses an advanced AI system that governs its behavior patterns, reactions to players, and environmental interactions.
...
The Merpeople (Siren) spawns in specific biomes and conditions that match its natural habitat.
...
The Merpeople (Siren) has specific statistics that determine its capabilities and behavior in the game world.
...
The Merpeople (Siren) uses sophisticated AI to govern its behavior, reactions, and interactions with the world.
...
The Merpeople (Siren) spawns under specific conditions that match its natural habitat and ecological niche.
...
The Merpeople (Siren) engages in combat when threatened or when its territory is invaded. It uses a combination of melee attacks and defensive behaviors to protect itself.
...
When defeated, the Merpeople (Siren) drops various items that can be used for crafting, potion-making, or other purposes.
...
**Natural Habitat:**
The Merpeople (Siren) naturally inhabits Lakes, oceans.
...
/**
 * Entity implementation for the Merpeople (Siren).
 * A magical creature that can be encountered in the wizarding world.
 */
public class MerpeopleSirenEntity extends MagicalCreatureEntity {
    
    public static final EntityType<MerpeopleSirenEntity> TYPE = EntityType.Builder
        .of(MerpeopleSirenEntity::new, MobCategory.CREATURE)
        .sized(0.6f, 1.8f)
        .build("merpeople__siren_");
    
    public MerpeopleSirenEntity(EntityType<? extends MerpeopleSirenEntity> type, Level level) {
        super(type, level);
...
EntitySpawnPlacements.register(
    MerpeopleSirenEntity.TYPE,
    SpawnPlacements.Type.ON_GROUND,
    Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
    MerpeopleSirenEntity::checkSpawnRules
);
```
