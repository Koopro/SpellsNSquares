...
title: 'Chapter 181: Flobberworm'
description: Flobberworm - Implementation details and reference
tags:
  - iv menagerie
  - 13 harmless
---
...
The **Flobberworm** is a X (Harmless) magical creature that inhabits the wizarding world. A thick, brown worm This creature plays an important role in the ecosystem and provides various interactions for players, including combat encounters, taming opportunities, and resource gathering.

This chapter documents the complete implementation of the Flobberworm, including AI behaviors, spawning mechanics, combat systems, taming requirements, and all creature-specific properties that make it unique in the mod.

## Implementation Details

### Creature Classification

The Flobberworm is classified as **X (Harmless)** according to the Ministry of Magic's classification system. This classification determines how dangerous the creature is and what precautions players should take when encountering it.
...
The Flobberworm uses an advanced AI system that governs its behavior patterns, reactions to players, and environmental interactions.
...
The Flobberworm spawns in specific biomes and conditions that match its natural habitat.
...
The Flobberworm has specific statistics that determine its capabilities and behavior in the game world.
...
The Flobberworm uses sophisticated AI to govern its behavior, reactions, and interactions with the world.
...
The Flobberworm spawns under specific conditions that match its natural habitat and ecological niche.
...
The Flobberworm engages in combat when threatened or when its territory is invaded. It uses a combination of melee attacks and defensive behaviors to protect itself.
...
When defeated, the Flobberworm drops various items that can be used for crafting, potion-making, or other purposes.
...
**Natural Habitat:**
The Flobberworm naturally inhabits Damp ditches.
...
| **Classification** | X (Harmless) | Ministry classification |
...
/**
 * Entity implementation for the Flobberworm.
 * A magical creature that can be encountered in the wizarding world.
 */
public class FlobberwormEntity extends MagicalCreatureEntity {
    
    public static final EntityType<FlobberwormEntity> TYPE = EntityType.Builder
        .of(FlobberwormEntity::new, MobCategory.CREATURE)
        .sized(0.6f, 1.8f)
        .build("flobberworm");
    
    public FlobberwormEntity(EntityType<? extends FlobberwormEntity> type, Level level) {
        super(type, level);
    ...
    public CreatureClassification getClassification() {
        return CreatureClassification.X__HARMLESS_;
    }
}
```
...
EntitySpawnPlacements.register(
    FlobberwormEntity.TYPE,
    SpawnPlacements.Type.ON_GROUND,
    Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
    FlobberwormEntity::checkSpawnRules
);
```
