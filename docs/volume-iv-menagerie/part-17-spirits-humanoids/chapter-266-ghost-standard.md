...
title: 'Chapter 266: Ghost (Standard)'
description: Ghost (Standard) - Implementation details and reference
tags:
  - iv menagerie
...
The **Ghost (Standard)** is a Spirits & Humanoids magical creature that inhabits the wizarding world. The spirit of a deceased wizard This creature plays an important role in the ecosystem and provides various interactions for players, including combat encounters, taming opportunities, and resource gathering.

This chapter documents the complete implementation of the Ghost (Standard), including AI behaviors, spawning mechanics, combat systems, taming requirements, and all creature-specific properties that make it unique in the mod.

## Implementation Details

### Creature Classification

The Ghost (Standard) is classified as **Spirits & Humanoids** according to the Ministry of Magic's classification system. This classification determines how dangerous the creature is and what precautions players should take when encountering it.
...
The Ghost (Standard) uses an advanced AI system that governs its behavior patterns, reactions to players, and environmental interactions.
...
The Ghost (Standard) spawns in specific biomes and conditions that match its natural habitat.
...
The Ghost (Standard) has specific statistics that determine its capabilities and behavior in the game world.
...
The Ghost (Standard) uses sophisticated AI to govern its behavior, reactions, and interactions with the world.
...
The Ghost (Standard) spawns under specific conditions that match its natural habitat and ecological niche.
...
The Ghost (Standard) engages in combat when threatened or when its territory is invaded. It uses a combination of melee attacks and defensive behaviors to protect itself.
...
When defeated, the Ghost (Standard) drops various items that can be used for crafting, potion-making, or other purposes.
...
**Natural Habitat:**
The Ghost (Standard) naturally inhabits Various.
...
/**
 * Entity implementation for the Ghost (Standard).
 * A magical creature that can be encountered in the wizarding world.
 */
public class GhostStandardEntity extends MagicalCreatureEntity {
    
    public static final EntityType<GhostStandardEntity> TYPE = EntityType.Builder
        .of(GhostStandardEntity::new, MobCategory.CREATURE)
        .sized(0.6f, 1.8f)
        .build("ghost__standard_");
    
    public GhostStandardEntity(EntityType<? extends GhostStandardEntity> type, Level level) {
        super(type, level);
...
EntitySpawnPlacements.register(
    GhostStandardEntity.TYPE,
    SpawnPlacements.Type.ON_GROUND,
    Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
    GhostStandardEntity::checkSpawnRules
);
```
