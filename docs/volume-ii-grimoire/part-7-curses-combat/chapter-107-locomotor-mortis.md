...
title: 'Chapter 107: Locomotor Mortis (Leg Lock)'
description: Locomotor Mortis (Leg Lock) - Implementation details and reference
tags:
  - ii grimoire
  - 7 curses combat
---
...
The **Locomotor Mortis** spell is a fundamental component of the wizarding combat and utility system. This spell allows players to moves objects through air. Understanding its mechanics, requirements, and implementation is essential for creating an authentic magical experience.

This chapter provides comprehensive technical documentation for implementing Locomotor Mortis, including spell casting mechanics, wand movements, incantation requirements, mana consumption, cooldown systems, and integration with the broader spell framework.

## Implementation Details

### Spell Casting Mechanics

The Locomotor Mortis spell is cast through a combination of wand movement, incantation, and magical energy channeling. The implementation requires careful coordination between the client-side input system and server-side validation.
...
The Locomotor Mortis spell requires a specific wand movement pattern: "Point wand at target". This pattern must be executed correctly for the spell to activate. The pattern is tracked using gesture recognition algorithms that analyze the player's wand trajectory in 3D space.

**Incantation:**
The verbal component "Locomotor" must be spoken (or activated via keybind) during the casting window. The incantation timing is synchronized with the wand movement for optimal spell power.

### Spell Effects

The Locomotor Mortis spell produces specific magical effects when successfully cast.

**Canonical Effect:** Moves objects through air

**Spell Type:** Locomotion Charm

**Etymology:** Latin "loco" (place) + "motor" (mover)
...
The Locomotor Mortis spell generates distinctive visual effects that enhance player feedback and immersion:
...
- **Incantation Voice Line**: "Locomotor" spoken by player character
...
|----------|-------|-------------|
| **Spell Name** | Locomotor Mortis | Official name of the spell |
| **Incantation** | "Locomotor" | Verbal component required |
...
/**
 * Implementation of the Locomotor Mortis spell.
 * This spell allows wizards to moves objects through air.
 */
public class LocomotorMortisSpell extends BaseSpell {
    
    public static final String SPELL_ID = "locomotor_mortis";
    public static final String INCANTATION = "Locomotor";
    public static final int BASE_MANA_COST = 20;
    public static final int BASE_COOLDOWN = 5;
    public static final double BASE_RANGE = 20;
    
    public LocomotorMortisSpell() {
...
applySpellEffect(caster, target, targetPos);
        ...
        level.playSound(null, caster.getPosition(), 
            SoundRegistry.LOCOMOTORMORTIS_CAST.get(), 
...
public static final Spell LOCOMOTORMORTIS = register(
    new LocomotorMortisSpell(),
